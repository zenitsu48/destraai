...
app_dir=$(pwd)
target_path="$app_dir/extensions/destra-submodule/extensions/vscode"
link_path="$app_dir/extensions/destra-ref"
submodule_url="https://github.com/zenitsu48/destra-submodule.git"
submodule_path="$app_dir/extensions/destra-submodule"
...
if [ -d "$submodule_path" ]; then
    echo "Removing existing destra-submodule directory"
    execute "rm -rf $submodule_path" "Failed to remove existing destra-submodule directory"
fi
...
if [ -d "$submodule_path" ]; then
    execute "cd $submodule_path" "Failed to change directory to extensions/destra-submodule"
else
